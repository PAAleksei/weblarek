...
`getTotalCost(): number | null` - метод, для получение стоимости всех товаров в корзине.
...
`validateDataOfCostumer(): boolean` - метод, для валидации данных покупателя.
***
...
`api: IApi` - поле, для хранения переданного в конструктор класса, используемого для реализации методов класса `ApiData`.
```
